	...
	if r.status_code == 200:
		...
		if 'element_count' in json_data:
			...
			if ast_count > 0:
				...
				logger.info("No asteroids are going to hit earth today") #prints the info, that all is good.
		...
		if len(ast_hazardous) > 0:
			...
			logger.info("No asteroids close passing earth today") #prints info if no close asteroids passing

		if len(ast_safe) > 0:
			push_asteroids_arrays_to_db(request_date, ast_safe, 0)

	else:
		logger.error("Unable to get response from API. Response code: " + str(r.status_code) + " | content: " + str(r.text)) #error message
